...
public class FirebaseMessagingPlugin extends BroadcastReceiver
    ...
    private void onAttachedToEngine(Context context, BinaryMessenger binaryMessenger) {
        ...
        intentFilter.addAction(FlutterFirebaseMessagingService.ACTION_WAKE_UP_SCREEN);
    ...
    private static void userPowerManagerWakeup(@NonNull Activity activity) {
        ...
        PowerManager.WakeLock wakelock = pm.newWakeLock(PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP, "social_doctor_call:");
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        } else if (action.equals(FlutterFirebaseMessagingService.ACTION_WAKE_UP_SCREEN)) {
            try {
                mainActivity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);

                userPowerManagerWakeup(mainActivity);
                useWindowFlags(mainActivity);
                useActivityScreenMethods(mainActivity);
               ...
               /* if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O_MR1) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    ((KeyguardManager) mainActivity.getSystemService(Context.KEYGUARD_SERVICE)).requestDismissKeyguard(mainActivity, null);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
